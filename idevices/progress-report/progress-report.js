...
var $eXeInforme = {
    ...
    loadFromContentXml: function (mOption) {
        const isExeIndex = (document.documentElement && document.documentElement.id === 'exe-index');
            ...
            pagStructures.forEach(pagStruct => {
                ...
                odeComponents.forEach(comp => {
                    ...
                    if (htmlViewNode && htmlViewNode.textContent) {
                        const matchId = htmlViewNode.textContent.match(/data-evaluationid\s*=\s*['"]([^'"]+)['"]/)
                        if (matchId && matchId[1]) evaluationID = matchId[1]

                        const matchEval = htmlViewNode.textContent.match(/data-evaluationb(?:\s*=\s*['"]([^'"]*)['"])?/)
                        if (!matchEval || matchEval[1] === "true") evaluation = true
                    }
        ...
        pages.forEach(page => {
            ...
            if (page.components && page.components.length > 0) {
                ...
                page.components.forEach(component => {
                    ...
                    const isEvaluable = component.evaluation && component.evaluationID && $eXeInforme.options.evaluationID && $eXeInforme.options.evaluationID == component.evaluationID;
        ...
        pages.forEach(page => {
            ...
            if (page.components && page.components.length > 0) {
                ...
                page.components.forEach(component => {
                    ...
                    const isEvaluable = evaluation && evaluationID
    ...
    getDataStorage: function (id) {
        ...
        const parsed = $exeDevices.iDevice.gamification.helpers.isJsonString(localStorage.getItem(key));
        return (parsed && Array.isArray(parsed.activities)) ? parsed.activities : [];
    },
    ...
    addEvents: function () {
        ...
        $c.on("click", "#informeReboot", function (e) {
            ...
            if (confirm($eXeInforme.options.msgs.msgDelete)) {
                localStorage.removeItem("dataEvaluation-" + $eXeInforme.options.evaluationID);
        ...
        window.addEventListener('gamification-evaluation-saved', function (ev) {
            ...
            if (!targetEval || !eventEval || String(targetEval) !== String(eventEval)) return;
